...
def default_args(**kwargs):
    ...
    parser.add_argument("--max-words", default=100, type=int)
    parser.add_argument("--train-size", default=50000, type=int)
    parser.add_argument("--test-size", default=20000, type=int)
    parser.add_argument("--binary-labels", default=True, type=bool)
    parser.add_argument("--min-word-freq", default=2, type=int)
...
class WordTokenizer:
    """Word tokenizer that preserves contractions and words"""
    
    def __init__(self, vocab_size=10000, min_freq=2):
        self.word_index = {"<PAD>": 0, "<UNK>": 1}
        self.reverse_word_index = {0: "<PAD>", 1: "<UNK>"}
        self.vocab_size = vocab_size
        self.min_freq = min_freq
        
    def _tokenize(self, text):
        """Split text into words"""
        # Pattern to match words including those with apostrophes
    ...
    def build_vocab(self, texts):
        ...
        vocab_words = [word for word, count in word_counts.most_common(self.vocab_size - 2)
                      if count >= self.min_freq]
        
        # Create word index mapping
        for i, word in enumerate(vocab_words):
            idx = i + 2  # Start at 2 since 0 and 1 are reserved
            self.word_index[word] = idx
            self.reverse_word_index[idx] = word
    ...
    def encode(self, text, max_length=None):
        ...
        # Handle empty text
        if not words:
            return [1]  # Return UNK token to avoid empty sequences
            
        if max_length:
            words = words[:max_length]
        
        token_ids = [self.word_index.get(word, 1) for word in words]  # 1 is <UNK>
        return token_ids
    ...
    def batch_encode(self, texts, max_length=None):
        ...
        empty_count = sum(1 for seq in encoded if len(seq) == 0)
        if empty_count > 0:
            print(f"Warning: {empty_count} sequences are empty after preprocessing")
            # Add UNK token to empty sequences
            encoded = [seq if seq else [1] for seq in encoded]
...
def prepare_yelp_data(args):
    ...
    # First pass: determine the labels for all examples
    print("Determining labels for all examples...")
    train_positive_indices = []
    train_negative_indices = []
    
    # Process training set to separate positive and negative indices
    for i, example in enumerate(dataset["train"]):
        if args.binary_labels:
            # Convert to binary labels (1-2 stars = 0, 3-5 stars = 1)
            label = 1 if example["label"] >= 3 else 0
            if label == 1:
                train_positive_indices.append(i)
            else:
                train_negative_indices.append(i)
    
    test_positive_indices = []
    test_negative_indices = []
    
    # Process test set to separate positive and negative indices
    for i, example in enumerate(dataset["test"]):
        if args.binary_labels:
            label = 1 if example["label"] >= 3 else 0
            if label == 1:
                test_positive_indices.append(i)
            else:
                test_negative_indices.append(i)
    
    # Calculate how many samples per class
    train_samples_per_class = args.train_size // 2
    test_samples_per_class = args.test_size // 2
    
    print(f"Available training examples: {len(train_positive_indices)} positive, {len(train_negative_indices)} negative")
    print(f"Available test examples: {len(test_positive_indices)} positive, {len(test_negative_indices)} negative")
    print(f"Required samples per class: {train_samples_per_class} for training, {test_samples_per_class} for testing")
    
    # Check if we have enough samples
    if len(train_positive_indices) < train_samples_per_class or len(train_negative_indices) < train_samples_per_class:
        print("WARNING: Not enough samples for balanced training set. Using all available with replacement.")
        train_pos_sampled = np.random.choice(train_positive_indices, train_samples_per_class, replace=True)
        train_neg_sampled = np.random.choice(train_negative_indices, train_samples_per_class, replace=True)
    else:
        train_pos_sampled = np.random.choice(train_positive_indices, train_samples_per_class, replace=False)
        train_neg_sampled = np.random.choice(train_negative_indices, train_samples_per_class, replace=False)
    
    if len(test_positive_indices) < test_samples_per_class or len(test_negative_indices) < test_samples_per_class:
        print("WARNING: Not enough samples for balanced test set. Using all available with replacement.")
        test_pos_sampled = np.random.choice(test_positive_indices, test_samples_per_class, replace=True)
        test_neg_sampled = np.random.choice(test_negative_indices, test_samples_per_class, replace=True)
    else:
        test_pos_sampled = np.random.choice(test_positive_indices, test_samples_per_class, replace=False)
        test_neg_sampled = np.random.choice(test_negative_indices, test_samples_per_class, replace=False)
    
    # Combine the samples
    train_indices = np.concatenate([train_pos_sampled, train_neg_sampled])
    test_indices = np.concatenate([test_pos_sampled, test_neg_sampled])
    
    # Shuffle the indices to mix positive and negative examples
    ...
    for idx in train_indices:
        ...
        if args.binary_labels:
            # Convert to binary labels (1-2 stars = 0, 3-5 stars = 1)
            train_labels.append(1 if example["label"] >= 3 else 0)
        else:
            # Use original 1-5 star labels
            train_labels.append(example["label"] - 1)  # 0-4 based indexing
    ...
    for idx in test_indices:
        ...
        if args.binary_labels:
            test_labels.append(1 if example["label"] >= 3 else 0)
        else:
            test_labels.append(example["label"] - 1)
    ...
    test_pos = np.sum(test_labels == 1)
    test_neg = np.sum(test_labels == 0)
    
    print("\n===== Class Distribution =====")
    print(f"Training set ({len(train_labels)} samples):")
    print(f"  - Positive samples: {train_pos} ({train_pos/len(train_labels)*100:.1f}%)")
    print(f"  - Negative samples: {train_neg} ({train_neg/len(train_labels)*100:.1f}%)")
    
    print(f"\nTest set ({len(test_labels)} samples):")
    print(f"  - Positive samples: {test_pos} ({test_pos/len(test_labels)*100:.1f}%)")
    print(f"  - Negative samples: {test_neg} ({test_neg/len(test_labels)*100:.1f}%)")
    print("===============================\n")
    
    # Initialize and build word tokenizer (without stopword removal)
    print("Building vocabulary...")
    tokenizer = WordTokenizer(
        vocab_size=args.vocab_size, 
        min_freq=args.min_word_freq
    )
    tokenizer.build_vocab(train_texts)  # Build vocab from training data only
    
    # Tokenize texts
    print("Tokenizing texts...")
    train_tokens = tokenizer.batch_encode(train_texts, max_length=args.max_words)
    test_tokens = tokenizer.batch_encode(test_texts, max_length=args.max_words)
    
    # Get length statistics
    train_lengths = [len(seq) for seq in train_tokens]
    test_lengths = [len(seq) for seq in test_tokens]
    
    print(f"Sequence length statistics after processing:")
    print(f"Training set - Avg: {np.mean(train_lengths):.1f}, Min: {np.min(train_lengths)}, Max: {np.max(train_lengths)}")
    print(f"Test set - Avg: {np.mean(test_lengths):.1f}, Min: {np.min(test_lengths)}, Max: {np.max(test_lengths)}")
    
    word_index = tokenizer.get_vocab()
    reverse_word_index = {value: key for key, value in word_index.items()}
    
    print(f"Training samples: {len(train_tokens)}")
    print(f"Testing samples: {len(test_tokens)}")
    print(f"Vocabulary size: {len(word_index)}")
    
    return (train_tokens, train_labels), (test_tokens, test_labels), word_index, reverse_word_index
...
def run_single_experiment(args, run_id=0):
    ...
    (train_x, train_y), (test_x, test_y), word_index, id_to_word = prepare_yelp_data(args)

    # Create symbols
    symbols = [id_to_word.get(i, f"UNK_{i}") for i in range(max(id_to_word.keys()) + 1)]
    ...
    for epoch in range(args.epochs):
        ...
        train_false_pos = np.sum((train_pred == 1) & (train_y == 0))
        train_false_neg = np.sum((train_pred == 0) & (train_y == 1))
        test_false_pos = np.sum((test_pred == 1) & (test_y == 0))
        test_false_neg = np.sum((test_pred == 0) & (test_y == 1))
        
        # Print error statistics
        print("\nMisclassification Statistics:")
        print(f"Train: {train_incorrect}/{len(train_y)} misclassified ({train_incorrect/len(train_y)*100:.2f}%)")
        print(f"  - False Positives: {train_false_pos}")
        print(f"  - False Negatives: {train_false_neg}")
        print(f"Test: {test_incorrect}/{len(test_y)} misclassified ({test_incorrect/len(test_y)*100:.2f}%)")
        print(f"  - False Positives: {test_false_pos}")
        print(f"  - False Negatives: {test_false_neg}")
